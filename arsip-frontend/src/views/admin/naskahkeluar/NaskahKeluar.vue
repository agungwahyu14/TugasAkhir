                        ...
                        <tbody>
                            <template v-if="naskahs.length > 0">
                                <tr v-for="(naskah, index) in filteredNaskahs" :key="naskah.id">
                                    ...
                                    <td class="px-6 align-middle border border-solid py-3 text-xs whitespace-nowrap">
                                        {{ naskah.tujuan == 'kadis' ? 'Kepala Dinas' : naskah.tujuan == 'sekdis' ?
                                            'Sekretasris Dinas' :
                                            'Kepala Bagian' }}
                                    </td>
                                    <td class="px-6 align-middle border border-solid py-3 text-xs whitespace-nowrap">
                                    ...
                                    <td class="px-6 align-middle border border-solid py-3 text-xs whitespace-nowrap">
                                        ...
                                        <router-link
                                            :to="`/admin/naskahkeluar/detail-naskah-keluar/${naskah.id_naskah_keluar}`"
                                            class="text-white rounded bg-orange-500 text-xs px-4 py-2 mr-2">
...
export default {
    ...
    methods: {
        ...
        fetchPagination(page = 1) {
            ...
            axios
                .get(`http://127.0.0.1:8000/api/naskah-keluars?page=${page}&search=${this.searchQuery}`, {
                    headers: {
